...
class DatabaseSettings(BaseModel):
    ...
    path: Path = Field(
        default=Path("data/recon.db"),
        description="مسار ملف SQLite"
    )
    pool_size: int = Field(
        default=5,
        ge=1,
        le=20,
        description="حجم connection pool"
    )
    timeout: float = Field(
        default=30.0,
        ge=1.0,
        le=300.0,
        description="timeout للاستعلامات بالثواني"
    )
    wal_mode: bool = Field(
        default=True,
        description="استخدام WAL mode للأداء"
    )
...
class APISettings(BaseModel):
    ...
    workers: int = Field(
        ...
        ge=1,
        le=32,
        description="عدد workers"
    )
    cors_origins: List[str] = Field(
        default=["*"],
        description="CORS origins المسموحة"
    )
...
class PipelineSettings(BaseModel):
    ...
    max_concurrent: int = Field(
        ...
        ge=1,
        le=500,
        description="الحد الأقصى للعمليات المتزامنة"
    )
    stage_timeout: float = Field(
        default=3600.0,
        ge=60.0,
        description="timeout للمرحلة بالثواني"
    )
    retry_attempts: int = Field(
        default=3,
        ge=0,
        le=10,
        description="عدد محاولات الإعادة"
    )
    retry_delay: float = Field(
        default=5.0,
        ge=1.0,
        description="التأخير بين المحاولات"
    )
    parallel_stages: bool = Field(
        default=True,
        description="تنفيذ المراحل المستقلة بالتوازي"
    )
    save_intermediate: bool = Field(
        default=True,
        description="حفظ النتائج الوسيطة"
    )
    gc_between_stages: bool = Field(
        default=True,
        description="تشغيل GC بين المراحل"
    )
...
class HTTPSettings(BaseModel):
    ...
    timeout: float = Field(
        ...
        ge=5.0,
        le=300.0,
        description="timeout للطلبات"
    )
    max_connections: int = Field(
        default=100,
        ge=10,
        le=1000,
        description="الحد الأقصى للاتصالات"
    )
    max_per_host: int = Field(
        default=10,
        ge=1,
        le=100,
        description="الحد الأقصى للاتصالات لكل host"
    )
    verify_ssl: bool = Field(
        default=True,
        description="التحقق من SSL"
    )
    follow_redirects: bool = Field(
        default=True,
        description="متابعة redirects"
    )
    max_redirects: int = Field(
        default=10,
        ge=0,
        le=50,
        description="الحد الأقصى للـ redirects"
    )
    user_agent: str = Field(
        default="recon-cli/1.0",
...
class DNSSettings(BaseModel):
    ...
    resolvers: List[str] = Field(
        ...
        description="قائمة resolvers"
    )
    timeout: float = Field(
        default=5.0,
        ge=1.0,
        le=30.0,
        description="timeout للاستعلام"
    )
    retries: int = Field(
        default=2,
        ge=0,
        le=5,
        description="عدد المحاولات"
    )
    cache_ttl: int = Field(
        default=300,
        ge=60,
        le=86400,
        description="TTL للـ cache بالثواني"
    )
    max_concurrent: int = Field(
        default=100,
        ge=10,
        le=500,
        description="استعلامات متزامنة"
    )
...
class ToolsSettings(BaseModel):
    ...
    path: Path = Field(
        ...
        description="مسار الأدوات"
    )
    timeout: float = Field(
        default=300.0,
        ge=30.0,
        description="timeout للأدوات"
    )
...
class SecretsSettings(BaseModel):
    ...
    min_entropy: float = Field(
        ...
        ge=2.0,
        le=5.0,
        description="الحد الأدنى للـ entropy"
    )
...
class LoggingSettings(BaseModel):
    ...
    format: Literal["text", "json"] = Field(
        default="text",
        description="تنسيق السجلات"
    )
    file: Optional[Path] = Field(
        default=None,
        description="ملف السجلات"
    )
    max_size: int = Field(
        default=10 * 1024 * 1024,  # 10MB
        description="الحد الأقصى لحجم الملف"
    ...
    backup_count: int = Field(
        ...
        ge=0,
        le=100,
        description="عدد النسخ الاحتياطية"
    )
...
class JobsSettings(BaseModel):
    ...
    base_path: Path = Field(
        default=Path("jobs"),
        description="مسار الـ jobs"
    )
    max_concurrent_jobs: int = Field(
        default=5,
        ge=1,
        le=20,
        description="الحد الأقصى للـ jobs المتزامنة"
    )
    cleanup_after_days: int = Field(
        default=30,
        ge=1,
        description="حذف الـ jobs القديمة بعد أيام"
    )
...
class Settings(BaseSettings):
    ...
    config_path: Path = Field(
        default=Path("config"),
        description="مسار ملفات الإعدادات"
    )
    data_path: Path = Field(
        default=Path("data"),
        description="مسار البيانات"
    )

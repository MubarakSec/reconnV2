...
DEFAULT_PROFILES_CONTENT = """{
  ...
  "ultra-deep": {
    ...
    "runtime": {
      ...
      "ffuf_maxtime": 300,
      "nuclei_batch_size": 8,
      "nuclei_batch_timeout_base": 420,
      "nuclei_batch_timeout_per_target": 60,
      "nuclei_batch_timeout_max": 2400,
      "nuclei_single_timeout": 1800,
      "enable_auth_discovery": true,
      "auth_discovery_max_urls": 80,
      "auth_discovery_max_forms": 120,
      "enable_js_intel": true,
      "js_intel_max_files": 100,
      "js_intel_max_urls": 400,
      "enable_api_recon": true,
      "api_recon_max_hosts": 120,
      "enable_api_schema_probe": true,
      "api_schema_max_specs": 60,
      "api_schema_max_endpoints": 600,
      "enable_param_mining": true,
      "param_mining_max_urls": 400,
      "param_mining_max_params": 150,
      "param_mining_mutations_per_param": 15,
      "enable_html_form_mining": true,
      "html_form_max_urls": 150,
      "html_form_max_forms": 400,
      "html_form_max_params": 140,
      "enable_waf_probe": true,
      "waf_probe_max_urls": 40,
      "enable_takeover": true,
      "takeover_require_cname": true,
      "enable_dalfox": true,
      "dalfox_max_urls": 50,
      ...
      "nmap_max_hosts": 300,
      "nmap_udp": true,
      "nmap_udp_top_ports": 500,
      "runtime_crawl_max_urls": 120,
      "runtime_crawl_per_host_limit": 6,
      "runtime_crawl_concurrency": 6,
      "runtime_crawl_timeout": 45,
      "max_fuzz_hosts": 30,
      "enable_fuzz": true,
      "enable_runtime_crawl": true,
      "enable_screenshots": true,
...
class RuntimeConfig:
    ...
    parallel_stages: bool = field(
        default_factory=lambda: os.environ.get("RECON_PARALLEL_STAGES", "0") not in {"0", "false", "False"}
    )
    ...
    fuzz_custom_max_words: int = int(os.environ.get("RECON_FUZZ_CUSTOM_MAX_WORDS", 1500))
    fuzz_combined_max_words: int = int(os.environ.get("RECON_FUZZ_COMBINED_MAX_WORDS", 6000))
    ...
    stage_heartbeat_seconds: int = int(os.environ.get("RECON_STAGE_HEARTBEAT_SECONDS", 1200))
    ...
    enable_screenshot_ocr: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_SCREENSHOT_OCR", "0") not in {"0", "false", "False"})
    ...
    enable_api_schema_probe: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_API_SCHEMA_PROBE", "0") not in {"0", "false", "False"})
    enable_param_mining: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_PARAM_MINING", "0") not in {"0", "false", "False"})
    enable_html_form_mining: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_HTML_FORM_MINING", "0") not in {"0", "false", "False"})
    enable_graphql_recon: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_GRAPHQL_RECON", "0") not in {"0", "false", "False"})
    enable_graphql_exploit: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_GRAPHQL_EXPLOIT", "0") not in {"0", "false", "False"})
    enable_oauth_discovery: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_OAUTH_DISCOVERY", "0") not in {"0", "false", "False"})
    enable_ws_grpc_discovery: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_WS_GRPC_DISCOVERY", "0") not in {"0", "false", "False"})
    enable_upload_probe: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_UPLOAD_PROBE", "0") not in {"0", "false", "False"})
    enable_cms_scan: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_CMS_SCAN", "0") not in {"0", "false", "False"})
    enable_vhost: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_VHOST", "0") not in {"0", "false", "False"})
    enable_subdomain_permute: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_SUBDOMAIN_PERMUTE", "0") not in {"0", "false", "False"})
    enable_cloud_discovery: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_CLOUD_DISCOVERY", "0") not in {"0", "false", "False"})
    enable_ct_pivot: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_CT_PIVOT", "0") not in {"0", "false", "False"})
    enable_asn_pivot: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_ASN_PIVOT", "0") not in {"0", "false", "False"})
    enable_waf_probe: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_WAF_PROBE", "0") not in {"0", "false", "False"})
    enable_takeover: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_TAKEOVER", "0") not in {"0", "false", "False"})
    takeover_require_cname: bool = field(default_factory=lambda: os.environ.get("RECON_TAKEOVER_REQUIRE_CNAME", "1") not in {"0", "false", "False"})
    enable_authenticated_scan: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_AUTH_SCAN", "0") not in {"0", "false", "False"})
    ...
    enable_verification: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_VERIFICATION", "0") not in {"0", "false", "False"})
    enable_exploit_validation: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_EXPLOIT_VALIDATION", "0") not in {"0", "false", "False"})
    enable_extended_validation: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_EXTENDED_VALIDATION", "0") not in {"0", "false", "False"})
    extended_validation_max_duration: int = int(os.environ.get("RECON_EXTENDED_VALIDATION_MAX_DURATION", 7200))
    extended_validation_max_probes: int = int(os.environ.get("RECON_EXTENDED_VALIDATION_MAX_PROBES", 500))
    enable_oast_validation: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_OAST_VALIDATION", "1") not in {"0", "false", "False"})
    ...
    enable_redirect_validation: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_REDIRECT_VALIDATION", "1") not in {"0", "false", "False"})
    redirect_max_urls: int = int(os.environ.get("RECON_REDIRECT_MAX_URLS", 40))
    enable_idor_validator: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_IDOR_VALIDATOR", "1") not in {"0", "false", "False"})
    idor_validator_max_candidates: int = int(os.environ.get("RECON_IDOR_VALIDATOR_MAX_CANDIDATES", 40))
    idor_validator_max_per_host: int = int(os.environ.get("RECON_IDOR_VALIDATOR_MAX_PER_HOST", 8))
    idor_validator_min_score: int = int(os.environ.get("RECON_IDOR_VALIDATOR_MIN_SCORE", 60))
    idor_validator_timeout: int = int(os.environ.get("RECON_IDOR_VALIDATOR_TIMEOUT", 10))
    idor_validator_rps: float = float(os.environ.get("RECON_IDOR_VALIDATOR_RPS", 0))
    idor_validator_per_host_rps: float = float(os.environ.get("RECON_IDOR_VALIDATOR_PER_HOST_RPS", 0))
    enable_ssrf_validator: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_SSRF_VALIDATOR", "1") not in {"0", "false", "False"})
    ssrf_validator_max_urls: int = int(os.environ.get("RECON_SSRF_VALIDATOR_MAX_URLS", 25))
    ssrf_validator_max_per_host: int = int(os.environ.get("RECON_SSRF_VALIDATOR_MAX_PER_HOST", 6))
    ssrf_validator_min_score: int = int(os.environ.get("RECON_SSRF_VALIDATOR_MIN_SCORE", 40))
    ssrf_validator_timeout: int = int(os.environ.get("RECON_SSRF_VALIDATOR_TIMEOUT", 10))
    ssrf_validator_rps: float = float(os.environ.get("RECON_SSRF_VALIDATOR_RPS", 0))
    ssrf_validator_per_host_rps: float = float(os.environ.get("RECON_SSRF_VALIDATOR_PER_HOST_RPS", 0))
    ssrf_validator_enable_oast: bool = field(default_factory=lambda: os.environ.get("RECON_SSRF_VALIDATOR_ENABLE_OAST", "1") not in {"0", "false", "False"})
    ssrf_validator_oast_wait_seconds: int = int(os.environ.get("RECON_SSRF_VALIDATOR_OAST_WAIT_SECONDS", 45))
    ssrf_validator_oast_poll_interval: int = int(os.environ.get("RECON_SSRF_VALIDATOR_OAST_POLL_INTERVAL", 5))
    ssrf_validator_enable_internal: bool = field(default_factory=lambda: os.environ.get("RECON_SSRF_VALIDATOR_ENABLE_INTERNAL", "1") not in {"0", "false", "False"})
    enable_open_redirect_validator: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_OPEN_REDIRECT_VALIDATOR", "1") not in {"0", "false", "False"})
    open_redirect_validator_max_urls: int = int(os.environ.get("RECON_OPEN_REDIRECT_VALIDATOR_MAX_URLS", 30))
    open_redirect_validator_max_per_host: int = int(os.environ.get("RECON_OPEN_REDIRECT_VALIDATOR_MAX_PER_HOST", 6))
    open_redirect_validator_min_score: int = int(os.environ.get("RECON_OPEN_REDIRECT_VALIDATOR_MIN_SCORE", 40))
    open_redirect_validator_timeout: int = int(os.environ.get("RECON_OPEN_REDIRECT_VALIDATOR_TIMEOUT", 10))
    open_redirect_validator_rps: float = float(os.environ.get("RECON_OPEN_REDIRECT_VALIDATOR_RPS", 0))
    open_redirect_validator_per_host_rps: float = float(os.environ.get("RECON_OPEN_REDIRECT_VALIDATOR_PER_HOST_RPS", 0))
    enable_auth_bypass_validator: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_AUTH_BYPASS_VALIDATOR", "1") not in {"0", "false", "False"})
    auth_bypass_validator_max_urls: int = int(os.environ.get("RECON_AUTH_BYPASS_VALIDATOR_MAX_URLS", 25))
    auth_bypass_validator_max_per_host: int = int(os.environ.get("RECON_AUTH_BYPASS_VALIDATOR_MAX_PER_HOST", 6))
    auth_bypass_validator_min_score: int = int(os.environ.get("RECON_AUTH_BYPASS_VALIDATOR_MIN_SCORE", 35))
    auth_bypass_validator_timeout: int = int(os.environ.get("RECON_AUTH_BYPASS_VALIDATOR_TIMEOUT", 10))
    auth_bypass_validator_rps: float = float(os.environ.get("RECON_AUTH_BYPASS_VALIDATOR_RPS", 0))
    auth_bypass_validator_per_host_rps: float = float(os.environ.get("RECON_AUTH_BYPASS_VALIDATOR_PER_HOST_RPS", 0))
    auth_bypass_validator_enable_forced_browse: bool = field(default_factory=lambda: os.environ.get("RECON_AUTH_BYPASS_VALIDATOR_ENABLE_FORCED_BROWSE", "1") not in {"0", "false", "False"})
    auth_bypass_validator_enable_privilege_boundary: bool = field(default_factory=lambda: os.environ.get("RECON_AUTH_BYPASS_VALIDATOR_ENABLE_PRIVILEGE_BOUNDARY", "1") not in {"0", "false", "False"})
    enable_secret_exposure_validator: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_SECRET_EXPOSURE_VALIDATOR", "1") not in {"0", "false", "False"})
    secret_exposure_validator_max_findings: int = int(os.environ.get("RECON_SECRET_EXPOSURE_VALIDATOR_MAX_FINDINGS", 40))
    secret_exposure_validator_min_score: int = int(os.environ.get("RECON_SECRET_EXPOSURE_VALIDATOR_MIN_SCORE", 40))
    secret_exposure_validator_timeout: int = int(os.environ.get("RECON_SECRET_EXPOSURE_VALIDATOR_TIMEOUT", 10))
    secret_exposure_validator_rps: float = float(os.environ.get("RECON_SECRET_EXPOSURE_VALIDATOR_RPS", 0))
    secret_exposure_validator_per_host_rps: float = float(os.environ.get("RECON_SECRET_EXPOSURE_VALIDATOR_PER_HOST_RPS", 0))
    enable_lfi_validation: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_LFI_VALIDATION", "1") not in {"0", "false", "False"})
    lfi_max_urls: int = int(os.environ.get("RECON_LFI_MAX_URLS", 40))
    enable_header_validation: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_HEADER_VALIDATION", "1") not in {"0", "false", "False"})
    header_validation_max_urls: int = int(os.environ.get("RECON_HEADER_VALIDATION_MAX_URLS", 30))
    ...
    wayback_max_per_target: int = int(os.environ.get("RECON_WAYBACK_MAX_PER_TARGET", 2000))
    wayback_fair_share: bool = field(default_factory=lambda: os.environ.get("RECON_WAYBACK_FAIR_SHARE", "1") not in {"0", "false", "False"})
    ...
    correlation_attack_path_limit: int = int(os.environ.get("RECON_CORRELATION_ATTACK_PATH_LIMIT", 30))
    ...
    auth_discovery_per_host_rps: float = float(os.environ.get("RECON_AUTH_DISCOVERY_PER_HOST_RPS", 0))
    ...
    js_intel_per_host_rps: float = float(os.environ.get("RECON_JS_INTEL_PER_HOST_RPS", 0))
    js_intel_extract_dynamic_routes: bool = field(default_factory=lambda: os.environ.get("RECON_JS_INTEL_DYNAMIC_ROUTES", "1") not in {"0", "false", "False"})
    js_intel_extract_hidden_params: bool = field(default_factory=lambda: os.environ.get("RECON_JS_INTEL_HIDDEN_PARAMS", "1") not in {"0", "false", "False"})
    ...
    api_recon_per_host_rps: float = float(os.environ.get("RECON_API_RECON_PER_HOST_RPS", 0))
    api_recon_enrich_from_js: bool = field(default_factory=lambda: os.environ.get("RECON_API_RECON_ENRICH_FROM_JS", "1") not in {"0", "false", "False"})
    api_recon_max_enriched_paths: int = int(os.environ.get("RECON_API_RECON_MAX_ENRICHED_PATHS", 40))
    api_schema_max_specs: int = int(os.environ.get("RECON_API_SCHEMA_MAX_SPECS", 25))
    api_schema_max_endpoints: int = int(os.environ.get("RECON_API_SCHEMA_MAX_ENDPOINTS", 200))
    api_schema_max_per_host: int = int(os.environ.get("RECON_API_SCHEMA_MAX_PER_HOST", 0))
    ...
    api_schema_probe_safe_writes: bool = field(default_factory=lambda: os.environ.get("RECON_API_SCHEMA_SAFE_WRITES", "1") not in {"0", "false", "False"})
    api_schema_rps: float = float(os.environ.get("RECON_API_SCHEMA_RPS", 0))
    api_schema_per_host_rps: float = float(os.environ.get("RECON_API_SCHEMA_PER_HOST_RPS", 0))
    ...
    graphql_exploit_max_urls: int = int(os.environ.get("RECON_GRAPHQL_EXPLOIT_MAX_URLS", 30))
    graphql_exploit_max_per_host: int = int(os.environ.get("RECON_GRAPHQL_EXPLOIT_MAX_PER_HOST", 0))
    graphql_exploit_max_queries: int = int(os.environ.get("RECON_GRAPHQL_EXPLOIT_MAX_QUERIES", 6))
    graphql_exploit_timeout: int = int(os.environ.get("RECON_GRAPHQL_EXPLOIT_TIMEOUT", 10))
    graphql_exploit_rps: float = float(os.environ.get("RECON_GRAPHQL_EXPLOIT_RPS", 0))
    graphql_exploit_per_host_rps: float = float(os.environ.get("RECON_GRAPHQL_EXPLOIT_PER_HOST_RPS", 0))
    ...
    vhost_max_response_bytes: int = int(os.environ.get("RECON_VHOST_MAX_RESPONSE_BYTES", 65536))
    ...
    param_mining_generate_mutations: bool = field(default_factory=lambda: os.environ.get("RECON_PARAM_MINING_MUTATIONS", "1") not in {"0", "false", "False"})
    param_mining_mutations_per_param: int = int(os.environ.get("RECON_PARAM_MINING_MUTATIONS_PER_PARAM", 8))
    ...
    html_form_per_host_rps: float = float(os.environ.get("RECON_HTML_FORM_PER_HOST_RPS", 0))
    ...
    waf_probe_per_host_rps: float = float(os.environ.get("RECON_WAF_PROBE_PER_HOST_RPS", 0))
    enable_security_headers: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_SECURITY_HEADERS", "0") not in {"0", "false", "False"})
    security_headers_max_urls: int = int(os.environ.get("RECON_SECURITY_HEADERS_MAX_URLS", 40))
    security_headers_timeout: int = int(os.environ.get("RECON_SECURITY_HEADERS_TIMEOUT", 8))
    security_headers_rps: float = float(os.environ.get("RECON_SECURITY_HEADERS_RPS", 0))
    security_headers_per_host_rps: float = float(os.environ.get("RECON_SECURITY_HEADERS_PER_HOST_RPS", 0))
    enable_tls_hygiene: bool = field(default_factory=lambda: os.environ.get("RECON_ENABLE_TLS_HYGIENE", "0") not in {"0", "false", "False"})
    ...
    tls_hygiene_per_host_rps: float = float(os.environ.get("RECON_TLS_HYGIENE_PER_HOST_RPS", 0))
    ...
    auth_login_content_type: Optional[str] = os.environ.get("RECON_AUTH_LOGIN_CONTENT_TYPE")
    auth_login_success_regex: Optional[str] = os.environ.get("RECON_AUTH_LOGIN_SUCCESS_REGEX")
    auth_login_fail_regex: Optional[str] = os.environ.get("RECON_AUTH_LOGIN_FAIL_REGEX")
    auth_login_cookie_names: Optional[str] = os.environ.get("RECON_AUTH_LOGIN_COOKIE_NAMES")
    auth_login_timeout: int = int(os.environ.get("RECON_AUTH_LOGIN_TIMEOUT", 15))
    runtime_crawl_role_aware: bool = field(default_factory=lambda: os.environ.get("RECON_RUNTIME_CRAWL_ROLE_AWARE", "1") not in {"0", "false", "False"})
    runtime_crawl_max_auth_profiles: int = int(os.environ.get("RECON_RUNTIME_CRAWL_MAX_AUTH_PROFILES", 3))
    auth_apply_active_modules: bool = field(default_factory=lambda: os.environ.get("RECON_AUTH_APPLY_ACTIVE", "0") not in {"0", "false", "False"})
    auth_matrix_rps: float = float(os.environ.get("RECON_AUTH_MATRIX_RPS", 0))
    auth_matrix_per_host_rps: float = float(os.environ.get("RECON_AUTH_MATRIX_PER_HOST_RPS", 0))
    auth_matrix_max_targets: int = int(os.environ.get("RECON_AUTH_MATRIX_MAX_TARGETS", 120))
    auth_matrix_max_per_host: int = int(os.environ.get("RECON_AUTH_MATRIX_MAX_PER_HOST", 12))
    ...
    wpscan_plugins_detection: Optional[str] = os.environ.get("RECON_WPSCAN_PLUGINS_DETECTION")
    wpscan_random_user_agent: bool = field(default_factory=lambda: os.environ.get("RECON_WPSCAN_RANDOM_UA", "1") not in {"0", "false", "False"})

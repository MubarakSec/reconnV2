...
class HttpProbeStage(Stage):
    ...
    async def _fallback_probe_async(self, context: PipelineContext, hosts: List[str], seen_urls: Set[str]) -> None:
        try:
        ...
        config = HTTPClientConfig(max_concurrent=50, total_timeout=10, verify_ssl=context.runtime_config.verify_tls)
        tracker = context.results
        ...
        async with AsyncHTTPClient(config) as client:
            ...
            for (host, url, scheme), resp in zip(urls_to_check, responses):
                ...
                set_cookie_headers = [v for k, v in raw_headers.items() if k.lower() == "set-cookie" and v]
                header_values = [v for k, v in headers_lower.items() if k in ("x-powered-by", "server") and v]
    ...
    async def _probe_additional_paths_async(self, context: PipelineContext, hosts: List[str], seen_urls: Set[str]) -> None:
        if not hosts:
            return
        ...
        async with AsyncHTTPClient(config) as client:
            ...
            for (host, url, scheme, path), resp in zip(urls_to_check, responses):
                ...
                payload = {
                    ...
                    "tls": scheme == "https",
                    "response_time_ms": int(resp.elapsed * 1000) if getattr(resp, 'elapsed', None) else 0,
                    "etag": etag,
                    "last_modified": last_modified,
                }
    ...
    async def _probe_soft_404_async(self, context: PipelineContext, hosts: List[str]) -> None:
        runtime = context.runtime_config
        ...
        config = HTTPClientConfig(max_concurrent=20, total_timeout=timeout, verify_ssl=context.runtime_config.verify_tls)
        headers = {"User-Agent": "recon-cli soft-404"}
        soft_hosts: Dict[str, dict] = {}
        
        async with AsyncHTTPClient(config) as client:
            tasks = [client.get(url, headers=headers, follow_redirects=True) for _, url in urls_to_check]
            responses = await asyncio.gather(*tasks, return_exceptions=True)
            ...
            for (host, url), resp in zip(urls_to_check, responses):
                ...
                if enrich_utils.looks_like_soft_404(resp.status, body_snippet, title):
                    soft_hosts[host] = enrich_utils.get_soft_404_fingerprint(body, title)

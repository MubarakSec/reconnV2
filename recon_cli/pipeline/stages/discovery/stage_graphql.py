...
class GraphQLReconStage(Stage):
        ...
        async with AsyncHTTPClient(config, context=context) as client:
            for url in candidates:
                ...
                try:
                    ...
                    if not self._is_graphql_response(resp.status, resp.headers.get("Content-Type", ""), data):
                        continue
                    ...
                    if self._has_introspection_schema(data):
                        ...
                        for guess in self.COMMON_GQL_GUESSES:
                            try:
                                g_resp = await client.post(url, json={"query": guess}, headers=headers, follow_redirects=True)
                                g_data = self._safe_json(g_resp.body)
    ...
    def _collect_candidates(self, context: PipelineContext) -> List[str]:
        ...
        for url in js_graphql_endpoints:  # type: ignore[attr-defined]
            if isinstance(url, str) and url and url not in urls_seen:
                candidates.append(url); urls_seen.add(url)
        ...
        for host in signals.get("by_host", {}):
            if "api_surface" not in signals.get("by_host", {}).get(host, set()): continue
    ...
    def _is_graphql_response(status_code: int, content_type: str, data: Dict[str, object]) -> bool:
        ...
        if "application/json" in content_type.lower() or status_code in {200, 400}:

...
class ScannerStage(Stage):
    ...
    def execute(self, context: PipelineContext) -> None:
        ...
        for scanner in scanners:
            if scanner == "nuclei" and not getattr(
                context.runtime_config, "enable_nuclei", True
            ):
                context.logger.info("Nuclei scanner disabled via enable_nuclei flag")
                continue
            if scanner == "wpscan" and not getattr(
                context.runtime_config, "enable_wpscan", True
            ):
                context.logger.info("WPScan scanner disabled via enable_wpscan flag")
                continue

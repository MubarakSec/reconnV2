...
class PipelineProgress:
    ...
    def _create_stages_table(self) -> Table:
        ...
        for name, stage in self.stages.items():
            ...
            progress = f"{stage.completed}/{stage.total} ({stage.progress_percent:.0f}%)"
...
class MultiProgress:
    ...
    def update(self, name: str, completed: Optional[int] = None, advance: int = 0) -> None:
...
async def show_progress_for(
    ...
    with Progress(
        ...
        for item in items:
            result = await callback(item) if asyncio.iscoroutinefunction(callback) else callback(item)

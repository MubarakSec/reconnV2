...
class BaseWizard(Generic[T]):
    ...
    def _prompt_step(self, step: WizardStep) -> Any:
        ...
        if step.step_type == StepType.TEXT:
            ...
            idx = IntPrompt.ask(
                "Select option",
                default=step.choices.index(step.default) + 1 if step.default in step.choices else 1,
            )
            value = step.choices[idx - 1] if 1 <= idx <= len(step.choices) else step.default
            ...
            value = [step.choices[i - 1] for i in indices if 1 <= i <= len(step.choices)]
            ...
            self.console.print(f"[cyan]{step.prompt}[/cyan] (one per line, empty to finish)")
            items = []
            while True:
...
class ScanWizard(BaseWizard[Dict[str, Any]]):
    ...
    def _setup_steps(self) -> None:
        ...
        self.add_step(WizardStep(
            ...
            choices=[
                "dns_enum", "subdomain_enum", "port_scan",
                "web_discovery", "screenshot", "vulnerability_scan",
                "secrets_scan", "takeover_check",
            ],
            depends_on="profile",
    ...
    def _setup_steps(self) -> None:
        ...
        self.add_step(WizardStep(
            ...
            choices=[
                "dns_enum", "subdomain_enum", "port_scan",
                "web_discovery", "screenshot", "vulnerability_scan",
                "secrets_scan", "takeover_check", "auth_matrix",
                "idor_check", "correlation",
            ],
        ))
        ...
        self.add_step(WizardStep(
            ...
            step_type=StepType.MULTI_CHOICE,
            choices=[
                "amass", "subfinder", "dnsx", "naabu",
                "httpx", "nuclei", "ffuf", "katana",
                "gowitness", "dalfox", "sqlmap",
            ],
        ))
    ...
    def _setup_steps(self) -> None:
        ...
        self.add_step(WizardStep(
            ...
            choices=[
                "amass", "subfinder", "dnsx", "naabu",
                "httpx", "nuclei", "ffuf", "katana",
                "gowitness", "dalfox", "sqlmap", "uncover",
            ],
        ))

        ...
        if results:
            ...
            if hosts:
                ...
                for host in hosts[:50]:  # Limit to 50
                    hosts_data.append([
                        host.get('host', 'N/A')[:40],
                        host.get('ip', 'N/A'),
                        str(host.get('status_code', '-')),
                        host.get('source', 'N/A')
                    ])
            ...
            if vulns:
                ...
                for vuln in vulns[:100]:  # Limit to 100
                    vulns_data.append([
                        ...
                        vuln.get('name', 'N/A')[:30],
                        vuln.get('host', 'N/A')[:25],
                        vuln.get('template_id', 'N/A')[:20]
                    ])
        ...
        if results:
            ...
            if secrets:
                ...
                for secret in secrets[:50]:  # Limit to 50
                    secrets_data.append([
                        secret.get('secret_type', 'Unknown')[:25],
                        secret.get('location', 'N/A')[:35],
                        secret.get('severity', 'high').upper()
                    ])
...
<body>
    <header>
        ...
        <div class="subtitle">{self.config.company_name}</div>
    </header>
    
    <section class="info-section">
        <h2>معلومات الفحص</h2>
        <table class="info-table">
            <tr><td class="label">الهدف:</td><td>{target}</td></tr>
            <tr><td class="label">التاريخ:</td><td>{created[:10] if len(created) > 10 else created}</td></tr>
            <tr><td class="label">الملف الشخصي:</td><td>{profile}</td></tr>
        </table>
    </section>
    
    <section class="summary-section">
        <h2>ملخص تنفيذي</h2>
        <p>{self._generate_executive_summary(job_data, results)}</p>
    </section>
...
"""
            for host in hosts[:50]:
                ...
                <tr>
                    ...
                    <td>{host.get('status_code', '-')}</td>
                    <td>{host.get('source', 'N/A')}</td>
                </tr>
"""
            html += """
            </tbody>
...
"""
            for vuln in vulns[:100]:
                ...
                <tr>
                    ...
                    <td>{vuln.get('template_id', 'N/A')}</td>
                </tr>
"""
            html += """
            </tbody>
...
"""
            for secret in secrets[:50]:
                html += f"""
                <tr>
                    <td>{secret.get('secret_type', 'Unknown')}</td>
                    <td>{secret.get('location', 'N/A')}</td>
                    <td>{secret.get('severity', 'HIGH').upper()}</td>
                </tr>
"""
            html += """
            </tbody>
...
"""
        ...
        html += f"""
    <footer>
        <p>تم إنشاء هذا التقرير بواسطة {self.config.company_name} في {datetime.now().strftime('%Y-%m-%d %H:%M')}</p>
    </footer>
</body>
</html>
"""

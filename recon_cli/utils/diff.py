...
class ResultNormalizer:
    ...
    IGNORE_FIELDS = {
        "timestamp", "scan_time", "duration", "scan_id",
        "job_id", "worker_id", "temp_file",
    }
...
class ScanDiff:
        ...
        for category in all_categories:
            ...
            for key in set(old_cat.keys()) & set(new_cat.keys()):
                if old_cat[key] != new_cat[key]:
                    field_changes = self._compare_records(
                        old_cat[key], new_cat[key]
                    )
                    changes.append(Change(
    ...
    def _get_severity(
        ...
        """تحديد خطورة التغيير"""
        return self.SEVERITY_RULES.get(
    ...
    def format_report(
        ...
        for category, cat_changes in sorted(by_category.items()):
            ...
            for change in cat_changes[:10]:  # Limit per category
                ...
                sev = f"[{change.severity.value}]" if change.severity != Severity.INFO else ""

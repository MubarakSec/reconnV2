...
class HostStorage:
    ...
    def bulk_add(self, hosts: List[HostModel]) -> int:
        ...
        with get_connection() as conn:
            ...
            """,
                [
                    (h.job_id, h.hostname, h.ip, h.source, h.resolved, h.live)
                    for h in hosts
                ],
            )
    ...
    def get_by_job(self, job_id: str, limit: int = 1000) -> List[HostModel]:
        ...
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM hosts WHERE job_id = ? LIMIT ?", (job_id, limit)
            )
            return [
                HostModel(
                    id=row["id"],
...
class URLStorage:
    ...
    def get_by_job(self, job_id: str, limit: int = 1000) -> List[URLModel]:
        ...
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM urls WHERE job_id = ? LIMIT ?", (job_id, limit)
            )
            return [
                URLModel(
                    id=row["id"],
...
class SecretStorage:
    ...
    def get_by_job(self, job_id: str) -> List[SecretModel]:
        ...
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM secrets WHERE job_id = ?", (job_id,))
            return [
                SecretModel(
                    id=row["id"],
                    job_id=row["job_id"],

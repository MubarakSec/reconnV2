...
def test_scoring_stage_applies_rules(monkeypatch, tmp_path: Path):
    rules_path = tmp_path / "rules.json"
    rules_path.write_text(
        '[{"when":{"url_contains":"admin"},"add_tags":["custom:admin"]}]',
        encoding="utf-8",
    )
    monkeypatch.setattr(rules_engine, "RULES_PATH", rules_path)
    stage = ScoringStage()
    ...
    record.paths.results_jsonl.write_text(
        '{"type":"url","url":"https://ex.com/admin","hostname":"ex.com","tags":[]}\n',
        encoding="utf-8",
    )
    import json
    ...
    def mock_get_results(*args, **kwargs):
        return [
            json.loads(line)
            for line in record.paths.results_jsonl.read_text().splitlines()
            if line
        ]
